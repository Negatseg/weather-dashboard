...
const createWeatherCard = (cityName, weatherItem, index) => {
  if (index === 0){
        return '<div class="details">
                    <h2>${cityName} (${weatherItem.dx_txt.split(" ")[0]})</h2> 
          ...
          return '<li class="card">
                      <h3>(${weatherItem.dx_txt.split(" ")[0]})</h3>
...
const getCityCoordiantes = () => {
  ...
  fetch(GEOCODING_API_URL).then(res => res.json()).then(data => {
    ...
    alert("an error occurred while fetching teh coordiantes");
  });
}
